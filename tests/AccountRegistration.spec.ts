...
/*test('Complete User registration test', async () => {

    await homePage.clickMyAccount();
    await homePage.clickRegister();

    const confirmMessage: string | null = await registrationPage.completeRegistration({
    ...
    expect(confirmMessage).toContain("Created");
